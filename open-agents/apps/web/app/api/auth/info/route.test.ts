...
describe("GET /api/auth/info", () => {
  const originalLocalManagedTemplateTrial =
    process.env.OPEN_AGENTS_LOCAL_MANAGED_TEMPLATE_TRIAL;

  afterEach(() => {
    Object.assign(process.env, { NODE_ENV: originalNodeEnv });
    if (originalLocalManagedTemplateTrial === undefined) {
      delete process.env.OPEN_AGENTS_LOCAL_MANAGED_TEMPLATE_TRIAL;
    } else {
      process.env.OPEN_AGENTS_LOCAL_MANAGED_TEMPLATE_TRIAL =
        originalLocalManagedTemplateTrial;
    }
  });
  ...
  test("does not report local development users as managed template trial users by default", async () => {
  ...
  test("can opt local development into managed template trial limits", async () => {
    Object.assign(process.env, {
      NODE_ENV: "development",
      OPEN_AGENTS_LOCAL_MANAGED_TEMPLATE_TRIAL: "true",
    });
    const { GET } = await routeModulePromise;
